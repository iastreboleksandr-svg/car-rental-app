...
// Hardcoded brand suggestions for the autocomplete (MVP).
...
// The editable shape backing both create and edit modes.
...
// Fields we run validation against (description is optional, validated by length only).
...
// --- Validation ------------------------------------------------------------
// Each validator returns an i18n error *code* (resolved by the UI via
// t(`car.form.errors.<code>`)) or null when the field is valid.
...
// Returns an i18n error code or null.
