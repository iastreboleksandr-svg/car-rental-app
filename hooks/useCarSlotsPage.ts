...
export function useCarSlotsPage(carId: string) {
  ...
  const { data: car } = useQuery({
    queryKey: ['car', carId],
    queryFn: () => carService.getById(carId, token!),
  ...
  const {
    ...
    queryKey: slotsKey,
    queryFn: () => slotService.getAll(carId, token!),
    enabled: !!carId && hydrated && !!token,
  });
  ...
  // The backend GET currently returns an empty body, so we update the cache
  // directly instead of invalidating (which would wipe freshly created slots).
  const createMut = useMutation({
    mutationFn: (dto: CreateSlotDto) => slotService.create(carId, dto, token!),
    onSuccess: (created) => {
  ...
  const deleteMut = useMutation({
    mutationFn: (slotId: string) => slotService.remove(carId, slotId, token!),
  ...
  function handleDayClick(day: Date) {
    ...
    // Existing slot: bookings are read-only, available/blocked open the delete panel.
    if (slot) {
    ...
    // Empty day: build a range selection.
    setActiveSlot(null);
    if (!selStart || selEnd) {
      setSelStart(day);
      setSelEnd(null);
